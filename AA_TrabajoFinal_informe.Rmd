...
mutate(pregnant = ifelse(is.na(pregnant), 3.0, pregnant), 
       ...
       mass= ifelse(is.na(mass == 0),32.30, mass),
       pedigree= ifelse(is.na(pedigree == 0), 0.37, pedigree),
       age= ifelse(is.na(age == 0), 29.00, age),
       )
pima
```
...
-   método 1: Clasificador Bayesiano.
...
```{r}
library(class)
califKnn<-knn(train = train, test = test, cl = pima$diabetes, k=4)
califKnn
```
...
```{r}
# Se tienen que realizar modelos con las 2 metricas elegidas (knn y bayes). Luego se deben comparar.
```
...
```{r}
# Acá se debe calcular el accuracy, precision, recall, etc.
```
